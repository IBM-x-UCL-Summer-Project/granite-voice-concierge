...
Why no barge-in here: interrupting speech needs the microphone open while audio
plays. Many macOS devices (including built-in mic + speakers) refuse a
concurrent input+output stream and raise CoreAudio "PaMacCore err -50", which
truncates the speech. Barge-in is a separate, unit-tested feature of the
`command_control` package with its own harness (`demo_live_barge_in.py`); this
routines harness keeps playback uninterruptible so it runs cleanly everywhere.
