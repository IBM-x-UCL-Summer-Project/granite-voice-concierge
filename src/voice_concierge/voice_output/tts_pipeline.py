...
class OfflineTTS:
    ...
    def __init__(self, model_path: str = "en_GB-alan-medium.onnx", config_path: str = "en_GB-alan-medium.onnx.json"):
        """
        ...
        if not os.path.exists(self.model_path) or not os.path.exists(self.config_path):
            logging.warning("TTS model or config file is missing. Please ensure they are downloaded.")

    def speak(self, text: str, output_wav: str = "temp_output.wav", length_scale: float = 1.2) -> bool:
        """
...
if __name__ == "__main__":
    ...
    success = tts_engine.speak("Hello! I am your Granite voice concierge. I am ready to help.")
    if success:
