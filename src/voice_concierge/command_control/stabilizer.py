...
* it fleetingly maps silence or background noise to a grammar word, firing a
  command nobody spoke;
...
command must be seen twice within `confirm_window` before it is emitted, which
drops a lone hallucination, and once emitted the same command is refused for
`cooldown`, which drops the repeats. A command genuinely spoken again after the
cooldown still gets through.
...
class StableCommandSpotter:
    ...
    def process(self, frame: bytes) -> CommandEvent | None:
        ...
        if command == self._pending and now - self._pending_at <= self._confirm_window:
            self._pending = None
            self._emitted, self._emitted_at = command, now
