...
class _StructuredMemoryTarget(BaseModel):
    """Validated exact target generated for a memory mutation."""
...
class _StructuredListOperation(BaseModel):
    """Validated structured-list domain operation generated by the model."""
    ...
    @classmethod
    def require_non_blank_items(cls, value: tuple[str, ...]) -> tuple[str, ...]:
...
class _StructuredMemoryAction(BaseModel):
    ...
    target: _StructuredMemoryTarget | None = None
    list_operation: _StructuredListOperation | None = None
    requires_confirmation: bool

    @model_validator(mode="after")
    def require_valid_target_for_action(self) -> _StructuredMemoryAction:
        ...
        if (
            ...
            raise ValueError("memory store cannot target an existing ID")
        ...
        if self.action not in {"store", "update"}:
            raise ValueError("structured-list operation requires store or update")
        if self.content is not None:
            raise ValueError("structured-list operation must not duplicate content")
...
def _response_from_structured_payload(
    ...
    if payload.proposed_memory_action is not None:
        ...
        if action.target is not None:
            target = MemoryTarget(
                memory_id=action.target.memory_id,
                memory_key=action.target.memory_key,
