...
class MemoryStore:
    ...
    def get_memories_in_scope(self, scope: MemoryRecordScope) -> list[MemoryRecord]:
        ...
        if not isinstance(scope, MemoryRecordScope):
            raise TypeError("Exact memory lookup requires a MemoryRecordScope.")
        rows = self.cur.execute(
            """
