...
class TemporalExtractor:
    ...
    RELATIVE_PATTERNS = {
        ...
        r'\b(next|last)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b': None,
    ...
    @staticmethod
    def extract_iso_datetime(text: str, reference_time: Optional[datetime] = None) -> Optional[str]:
        """
        ...
        reference_time = reference_time or datetime.now(timezone.utc).replace(tzinfo=None)
    ...
    def _parse_relative_dates(
        ...
        match = re.search(
            r'(\d+)\s+(day|week|month|year)s?\s+ago\b', text_lower
        )
        if match:
            amount = int(match.group(1))
            unit = match.group(2)
    ...
    @staticmethod
    def _add_relative_time(
        reference_time: datetime, amount: int, unit: str
    ) -> str:
